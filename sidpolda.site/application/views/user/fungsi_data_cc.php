...
$(document).ready(function() {
    tabel = $('#datatabel').DataTable({
        ...
        "columns": [{
                "render": function(data, type, row) {
                    ...
                    html += '<a href="<?=$menu['url']?>/delete_data_cc/' + row.id +
    ...
    $('.tambah').on('click', function() {
        $('#mode').val("create");
        $('#id').val("");
        $("#form_bobot").attr("action", "<?= $menu['url'] . '/add_data_cc' ?>");
        $('#myLargeModalLabel').text('Tambah Input Data Clear Kejahatan');
        myModal.toggle();
    ...
    $('#datatabel tbody').on('click', '.editBtn', function() {
        var id = $(this).attr('id');
        var url = "<?= $menu['url'] . '/edit_select_cc' ?>";
        $('#mode').val("update");
        $('#id').val(id);
        $("#form_bobot").attr("action", "<?= $menu['url'] . '/update_data_cc' ?>");
    ...
    $('#Medium-modal').on('hidden.bs.modal', function() {
        $('#kejahatan, #kabupaten, #polres, #tgl_kejadian, #jumlah_cc').removeClass('is-valid');
        $('#kejahatan, #kabupaten, #polres, #tgl_kejadian, #jumlah_cc ').removeClass(
            'is-invalid');
        $('#kejahatan, #kabupaten, #polres, #tgl_kejadian, #jumlah_cc').val('');
    });
});
</script>